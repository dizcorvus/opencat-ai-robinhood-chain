...
```
┌─────────────────────────────────────────────────────────────────────────────┐
│                       OPENCAT AI COLOR PALETTE MATRIX                       │
├───────────────────────┬──────────────┬──────────────────┬───────────────────┤
...
6. **Risk Circuit Breaker, Honeypot Warnings, Stop Losses:** `Maneki-Neko Lucky Red (#E53935)`
7. **Realized Gains & Treasury Highlights:** `Golden Fortune (#FFD700)`

---

## 3. 🐱 Pixel Mascot & Terminal ASCII Art

### Standard OpenCat ASCII (Terminal TUI & Wizard Banner)

```text
       /\_____/\
      /  o   o  \      🐾 OPENCAT AI (ROBINHOOD CHAIN) 🐾
...
### Swag Sunglasses OpenCat (Command Center & Oracle)

```text
       /\_____/\
      /  ■   ■  \      🕶️ OPENCAT AI · COMMAND CENTER 🕶️
...
For CLI tools (`bin/opencat.js`, `src/cli/tui.ts`, `scripts/wizard.js`):

```typescript
export const OPENCAT_COLORS = {
  reset: '\x1b[0m',
  bold: '\x1b[1m',
  dim: '\x1b[2m',
...
- **Category:** `🐾 OPENCAT COMMAND CENTER`
- **Core Channels:**
  - `#opencat-control-room` — Main natural language chat, wallet balance, risk settings, and execution intents.
  - `#audit-on-demand` — Instant 12-point token audit upon pasting contract address (CA).
...
- **Identity:** OpenCat is a cool, laid-back pixel cat who has mastered on-chain DeFi.
