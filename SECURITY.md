# Security Policy - Athena AI (Robinhood Chain Edition)

Athena AI takes security and smart contract safety with the utmost seriousness. Because Athena interacts with real-world blockchain networks (Robinhood Chain EVM L2 #4663), Web3 DEX protocols (Uniswap V3), and automated execution mechanisms, strict security standards are embedded directly into our multi-agent architecture.

---

## 🛡️ Core Security Architecture & Safeguards

1. **`DRY_RUN` Mode Default**: Athena initializes in `DRY_RUN=true` mode by default. Real on-chain trading requires explicit enabling of `AUTO_EXECUTE_ENABLED=true` and supplying a verified `EVM_PRIVATE_KEY`.
...
4. **Circuit Breaker & Kill-Switch (`RiskEngineV2`)**: Automatic global drawdown caps, per-trade position limits, and instantaneous emergency kill-switches halt execution if risk thresholds are breached.
...
6. **REST API Guard (`ATHENA_API_KEY`)**: When exposing the native REST API (`src/api/server.ts`), requests can be authenticated via `X-Athena-Api-Key` or Bearer tokens with full CORS header isolation.

---

## 📋 Reporting a Vulnerability

If you discover a security vulnerability, key leak risk, or potential exploit vector within Athena AI, please **DO NOT** create a public GitHub Issue.

Instead, please report vulnerabilities privately:

- **Security Email**: Contact maintainers directly via `security@athena-ai.org` (or via private Discord DM to Core Maintainers in `#athena-control-room`).
