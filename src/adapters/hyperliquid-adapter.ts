/**
 * Hyperliquid Adapter — ETH Whale & Smart-Money Positioning Tracker
 *
 * Reads Hyperliquid L1 DEX data for ETH smart-money tracking:
 ...
 * - userFills per address → recent fills (incl. spot ETH/USDC) for whale flow detection
 *
...
export interface HyperliquidTradeFill {
  coin: string;        // perps: "ETH"; spot: "ETH/USDC" etc.
  isSpot: boolean;
  px: number;
  sz: number;
  usd: number;         // px * sz
...
export class HyperliquidAdapter {
  ...
  // Focused on ETH as the native asset of Robinhood Chain
  public readonly trackedAssets: string[] = ['ETH'];

  // PvP leaderboard cache (stats-data response is large: ~41k traders / tens of MB) —
  // fetched at most once per hour.
   ...
   * Spot coins come back as "ETH/USDC" style (isSpot = coin contains '/').
   */
