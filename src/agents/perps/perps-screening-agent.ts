 ...
 * 
 * Total = 0-100. Only setups with >= 80% are posted to #call-perps-futures.
...
export class PerpsScreeningAgent {
  ...
  private async evaluateSetup(market: HyperliquidMarketData): Promise<HyperliquidPerpsSignal | null> {
    ...
    // Macro(20) + Quant(20) + Risk(20) + Catalyst(15) + Technical(25) = 100
    // ============================================
    const totalScore = Math.max(0, Math.min(100, macroScore + quantScore + riskScore + catalystScore + technicalScore));
    ...
    const aiThesis = this.buildDeterministicThesis(
      ...
      macroScore, quantScore, riskScore, catalystScore, technicalScore,
  ...
  private buildDeterministicThesis(
    ...
    thesis += `Swarm: Macro ${macroScore}/20 | Quant ${quantScore}/20 | Risk ${riskScore}/20 | Catalyst ${catalystScore}/15 | Technical ${technicalScore}/25.`;
