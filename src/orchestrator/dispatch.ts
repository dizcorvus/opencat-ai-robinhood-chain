...
export function buildLPPayload(pool: LPPoolSignal): CallCardPayload {
  return {
    ...
    devHoldingPct: `Fee 1h $${pool.fee1hUsd.toFixed(0)}`,
    sniperPct: `${(pool.feesToTvlRatio1h * 100).toFixed(2)}% 1h Fee/TVL`,
    bundlerPct: `${pool.volumeToTvlRatio1h.toFixed(1)}x vol/TVL`,
    feeApr: `${(pool.feesToTvlRatio1h * 100).toFixed(2)}% (1h Fee/TVL)`,
