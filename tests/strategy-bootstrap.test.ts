...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'athena-boot-'));
const strategiesDir = path.join(tmp, 'strategies');

beforeEach(() => { fs.mkdirSync(strategiesDir, { recursive: true }); });
